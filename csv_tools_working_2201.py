...
def csv_save(df, file_path, ensure_folder=False, suffixes=None, run_stats=False):
    ...
    # Add suffixes only if they are not already part of the file name
    if suffixes:
        file_name, file_ext = os.path.splitext(file_path)
        existing_suffixes = set(file_name.split("_")[1:])  # Extract current suffixes
        new_suffixes = [suffix for suffix in suffixes if suffix not in existing_suffixes]
        if new_suffixes:  # Only add suffixes that are not already present
            suffix_string = "_".join(new_suffixes)  # Combine suffixes with underscores
